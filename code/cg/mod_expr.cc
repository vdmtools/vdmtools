...
#ifdef VDMPP
              if (vdm_CPP_isJAVA()) {
                retex = vdm_BC_GenBracketedExpr(assignexpr);
              }
              else
#endif // VDMPP
                retex = assignexpr;
                                           ...
                                           const TYPE_CPP_Name & succ_v)
{
  ...
  for (size_t i = 1; i <= len_altns; i++) {
    ...
    if (i == 1) {
      if (stmt.GetSequence(pos_CPP_CompoundStmt_stms).Length() == 1) {
...
Generic vdmcg::CGSeqEnumerationExpr (const TYPE_AS_SeqEnumerationExpr & rc1, const TYPE_CGMAIN_VT & vt)
{
  ...
  if (vdm_CPP_isJAVA()) {
    ...
    if (expr.IsEmpty()) {
      ...
      else if (iswstring || (possiblestringtype && isstringexpr)) { // string
...
#ifdef VDMPP
      if (vdm_CPP_isJAVA()) {
        dtp = FindMapDomType(rType);
        rtp = FindMapRngType(rType);
      }
#endif // VDMPP
...
//  const TYPE_REP_TypeRep & rType (vt.GetRecord(pos_CGMAIN_VT_type));
  ...
  if (expr.Is(TAG_TYPE_AS_PrefixExpr))
  {
    ...
    TYPE_CPP_Expr bl_cast;
    if (IsSeqType(argtype)) {
      bl_cast = bl_expr;
    }
    else {
#ifdef VDMPP
      if (vdm_CPP_isJAVA()) {
        if (IsStringType(argtype)) {
          bl_cast = GenCastString(bl_expr);
        }
        else {
          bl_cast = GenCastSeq(bl_expr, Nil());
        }
      }
      else
#endif // VDMPP
...
#ifdef VDMPP
    if (vdm_CPP_isJAVA()) {
      if (IsPossibleStringType(argtype)) {
        ub = GenLenString_int(bl_cast);
      }
      else {
        ub = GenLen_int(bl_cast);
      }
    }
    else
#endif // VDMPP
  ...
  if (vdm_CPP_isJAVA()) {
    if (IsPossibleStringType(rType)) {
      append = vdm_BC_GenAsgnStmt(tmpSeq_v, vdm_BC_GenPlus(tmpSeq_v, elemExpr));
    }
    else {
      append = GenImpAppend(tmpSeq_v, elemExpr);
    }
  }
  else
#endif // VDMPP
...
#ifdef VDMPP
    if (vdm_CPP_isJAVA()) {
      cond = GenGetValue(pred_v, mk_REP_BooleanTypeRep());
    }
    else
#endif // VDMPP
      cond = GenGetValue(pred_v, predType);
                                                     ...
                                                     const TYPE_CGMAIN_VT & vt)
{
  ...
  if (!rb_expr.Is(TAG_TYPE_CPP_Identifier)) {
    ...
    rb.ImpConc (GenDeclInit_DS (resSType, resS_v, rb_expr));
  }
  ...
  TYPE_CPP_Expr sort_f;
#ifdef VDMPP
  if (vdm_CPP_isJAVA()) {
    sort_f = vdm_BC_GenFctCall (
                            vdm_BC_GenIdentifier (ASTAUX::MkId(L"UTIL.Sort")), mk_sequence(resS_v));
  }
  else
#endif // VDMPP
    sort_f = vdm_BC_GenFctCall (
                            vdm_BC_GenIdentifier (ASTAUX::MkId(L"CGUTIL::Sort")), mk_sequence(resS_v));

  SEQ<TYPE_CPP_Stmt> rb_inner;
  rb_inner.ImpConc(GenDeclInit_DS (bindLType, bindL_v, sort_f));
...
#ifdef VDMPP
    if (vdm_CPP_isJAVA()) {
      lbval = GenGetValue(GenExplicitCast(irep, lb_vq, lbt), irep);
    }
    else
#endif // VDMPP
    {
...
#ifdef VDMPP
    if (vdm_CPP_isJAVA()) {
     ubval = GenGetValue(GenExplicitCast(irep, ub_vq, ubt), irep);
    }
    else
#endif // VDMPP
    {
      ubval = GenGetValue(vdm_BC_GenCastExpr(its, ub_vq), irep);
    }
...
#ifdef VDMPP
    if (vdm_CPP_isJAVA()) {
      stmt = vdm_BC_GenExpressionStmt(GenSetInsertExpr(GenCastSetType(resVar_v), cast));
    }
    else
#endif // VDMPP
    {
      stmt = vdm_BC_GenAsgnStmt(resVar_v, GenSetInsertExpr(GenSetExpr(resVar_v), cast));
    }
  }
  else {
    stmt = GenSetInsert(resVar_v, cast);
  }
    ...
    if (IsNumType(n2tp)) {
      if (IsIntType(n2tp)) {
        ...
        if (n2_v.Is(TAG_TYPE_CPP_FctCall) &&
          ...
          if (n2_v.Is(TAG_TYPE_CPP_CastExpr) &&
...
Generic vdmcg::CGTokenConstExpr(const TYPE_AS_TokenConstructorExpr & expr, const TYPE_CGMAIN_VT & vt)
{
  ...
  TYPE_CPP_Expr tokenconst;
#ifdef VDMPP
  if (vdm_CPP_isJAVA()) {
    tokenconst = GenTokenExpr(tmpvar);
  }
  else
#endif // VDPP
  {
    tokenconst = vdm_BC_GenFctCall(vdm_BC_GenIdentifier(ASTAUX::MkId(L"mk_token")), mk_sequence(tmpvar));
  }
...
#ifdef VDMPP
      if (vdm_CPP_isJAVA()) {
        cast = GenExplicitCast(tp, tmpVar_v, rtp);
      }
      else
#endif // VDMPP
      {
        cast = GenCastType(tp, tmpVar_v);
      }
    }
    if (stmts.IsEmpty()) {
      return cast;
...
#ifdef VDMPP
    if (vdm_CPP_isJAVA()) {
      casting = GetIntVal(tmppar_v);
    }
    else
#endif //VDMPP
    {
      casting = GenCastType(nattype, tmppar_v);
    }
  }
...
Generic vdmcg::CGIfExpr (const TYPE_AS_IfExpr & rc1, const TYPE_CGMAIN_VT & vt)
{
...
#ifdef VDMPP
  if (vdm_CPP_isJAVA()) {
    cond = GenGetValue(getval, mk_REP_BooleanTypeRep());
  }
  else
#endif // VDMPP
  {
  ...
  for (size_t i = elif.Length (); i >= 1; i--) {
    ...
    TYPE_CPP_Expr elif_cond;
#ifdef VDMPP
    if (vdm_CPP_isJAVA()) {
      elif_cond = GenGetValue(nres2, mk_REP_BooleanTypeRep());
    }
    else
#endif // VDMPP
    {
...
//    TYPE_CPP_Stmt ifisint (vdm_BC_GenIfStmt(int_cond, rre, nil));
...
#ifdef VDMPP
    if (vdm_CPP_isJAVA()) {
      e_decl.ImpConc(GenDeclInit_DS(nattp, e_v, vdm_BC_GenCastExpr(GenNatType(),edom)));
    }
    else
#endif // VDMPP
    {
  ...
  TYPE_CPP_Stmt if_body;
#ifdef VDMPP
  if (vdm_CPP_isJAVA()) {
    TYPE_CPP_Expr seq_len (GenLen_int(s_v));
    ...
    if_body = vdm_BC_GenIfStmt(cond, rre, vdm_BC_GenBlock(mk_sequence(alt2)));
  }
  else
#endif // VDMPP
  {
...
#ifdef VDMPP
  if (vdm_CPP_isJAVA()) {
    TYPE_CPP_Stmt rerr(RunTime(L"Wrong arguments for '++'"));
    if (seqmapType.Is(TAG_TYPE_REP_UnionTypeRep)
        && (seqmapType.GetSet(pos_REP_UnionTypeRep_tps).Card() == 1) ) {
...
#endif // VDMPP
  { // C++
    TYPE_CPP_Stmt rerr(RunTime(L"A map or sequence was expected in sequence or map override"));
    if (seqmapType.Is(TAG_TYPE_REP_UnionTypeRep)
        && (seqmapType.GetSet(pos_REP_UnionTypeRep_tps).Card() == 1) ) {
      TYPE_REP_TypeRep tp (seqmapType.GetSet(pos_REP_UnionTypeRep_tps).GetElem());
      if (tp.Is(TAG_TYPE_REP_SeqTypeRep)) {
                                             ...
                                             const TYPE_CGMAIN_VT & rc2)
{
  ...
  if (!tmpRecType.Is(TAG_TYPE_REP_UnionTypeRep) ||
      (tmpRecType.GetSet(pos_REP_UnionTypeRep_tps) != posRecTypes)) {
    alt.ImpAppend(RunTime(L"A record was expected"));
  }
#endif // VDMSL
#ifdef VDMPP
  if (!tmpRecType.Is(TAG_TYPE_REP_UnionTypeRep) ||
      (tmpRecType.GetSet(pos_REP_UnionTypeRep_tps) != posRecTypes.Union(posORefTypes))) {
  ...
  if (vdm_CPP_isJAVA()) {
    if (tmpRecType.Is(TAG_TYPE_REP_CompositeTypeRep) && (posRecTypes.Card() == 1)) {
      ...
      for (bool bb = posRecTypes.First(t); bb; bb = posRecTypes.Next(t)) {
        ...
        if (alt.IsEmpty()) {
          alt = SEQ<TYPE_CPP_Stmt>().ImpAppend(l_asgn);
        }
        else {
          alt = SEQ<TYPE_CPP_Stmt>().ImpAppend(vdm_BC_GenIfStmt(GenIsThisRecord(ctr, tmpRec), 
          ...
          for (bool bb = posORefTypes.First(t); bb; bb = posORefTypes.Next(t)) {
            ...
            if (alt.IsEmpty()) {
              ...
              alt = SEQ<TYPE_CPP_Stmt>().ImpAppend(vdm_BC_GenIfStmt(if_cond,
                                                          vdm_BC_GenBlock(mk_sequence(l_asgn)),
                                                          vdm_BC_GenBlock(alt)));
            }
          }
        }
...
//
        ...
        for (bool bb = posRecTypes.First(t); bb; bb = posRecTypes.Next(t)) {
          ...
          if (alt.IsEmpty()) {
            alt = SEQ<TYPE_CPP_Stmt>().ImpAppend(asgn);
          }
          else {
            alt = SEQ<TYPE_CPP_Stmt>().ImpAppend(vdm_BC_GenIfStmt(isRecord,
                                                         vdm_BC_GenBlock(mk_sequence(asgn)),
                                                         vdm_BC_GenBlock(alt)));
          }
        }
      }
      ...
      if (tmpRecType.Is(TAG_TYPE_REP_ObjRefTypeRep) && (posORefTypes.Card() == 1)) {
        ...
        for (bool bb = posORefTypes.First(t); bb; bb = posORefTypes.Next(t)) {
          ...
          if (alt.IsEmpty()) {
            ...
            alt = SEQ<TYPE_CPP_Stmt>().ImpAppend(vdm_BC_GenIfStmt(if_cond, vdm_BC_GenBlock(mk_sequence(l_asgn)),
                                                        vdm_BC_GenBlock(alt)));
          }
        }
      }
                                       ...
                                       const SEQ<TYPE_AS_RecordModification> & mu_l)
{
  ...
  switch (postp_s.Card()) {
    case 0: {
      return SEQ<TYPE_CPP_Stmt>().ImpAppend(errstmt);
    }
    case 1: {
      ...
      if (vdm_CPP_isJAVA()) {
        ...
        for (size_t idx = 1; idx <= len_mu_l; idx++) {
          ...
          const Tuple tpl (CGExprExcl(newval, ASTAUX::MkId(L"val"), nil));
      ...
      { // C++
        ...
        for (size_t idx = 1; idx <= len_mu_l; idx++) {
          const TYPE_AS_RecordModification & rm (mu_l[idx]);
          const TYPE_CPP_Expr fieldno (GenRecGetFieldNo(t, rm.GetRecord(pos_AS_RecordModification_field)));
          const TYPE_AS_Expr & newval (rm.GetRecord(pos_AS_RecordModification_newexpr));
          const Tuple tpl (CGExprExcl(newval, ASTAUX::MkId(L"val"), nil));
          const TYPE_CPP_Expr & val (tpl.GetRecord(1));
          const SEQ<TYPE_CPP_Stmt> & val_stmt(tpl.GetSequence(2));
    ...
    default: {
      ...
      if (vdm_CPP_isJAVA()) {
        ...
        for (bool b1 = postp_s.First(ctr_g); b1; b1 = postp_s.Next(ctr_g)) {
          ...
          ifs = vdm_BC_GenIfStmt(isRecord, vdm_BC_GenBlock(stmt_l), vdm_BC_GenBlock(mk_sequence(ifs)));
        }
        return SEQ<TYPE_CPP_Stmt>().ImpAppend(ifs);
      }
      else
#endif // VDMPP
      { // C++
        ...
        case_l.ImpAppend(vdm_BC_GenDefaultStmt(
                            vdm_BC_GenBlock(mk_sequence(errstmt, vdm_BC_GenBreakStmt(Nil())))));
        return SEQ<TYPE_CPP_Stmt>().ImpAppend(vdm_BC_GenSwitchStmt(GenGetTag(rec), case_l));
      }
    }
  }
}
...
Generic vdmcg::CGIsExpr (const TYPE_AS_IsExpr & rc1, const TYPE_CGMAIN_VT & vt)
{
  ...
  { // type : AS`Name
#ifdef VDMPP
    if (vdm_CPP_isJAVA()) {
      TYPE_CPP_Expr argTmpCast (vdm_BC_GenCastExpr(
                                   vdm_BC_GenTypeSpecifier(
                                     vdm_BC_GenIdentifier(ASTAUX::MkId(L"Object"))), argTmp_v));
...
#endif // VDMPP
    {
      ...
      rb.ImpAppend (vdm_BC_GenAsgnStmt (resVar_v, GenBoolExpr(expr_c)));
    }
    return rb;
  }
}
...
#ifdef VDMPP
    if (vdm_CPP_isJAVA()) {
      decl.ImpConc(GenDecl_DS(maptype, tmp_v, vdm_BC_GenAsgnInit(GenCastMap(e_v))));
    }
    else
#endif // VDMPP
...
#ifdef VDMPP
      if (vdm_CPP_isJAVA()) {
        cond = GenGetValue(vdm_BC_GenCastExpr(GenBoolType(), pred_v), mk_REP_BooleanTypeRep());
      }
      else
#endif // VDMPP
      {
...
#endif // VDMPP
            rb.ImpConc(GenDeclSet(e1_v, e_v));
...
#endif // VDMPP
            rb.ImpConc(GenDeclSeq(e1_v, e_v));
