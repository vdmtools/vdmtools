...
// GenRecGetFieldOnThis
// meth : seq1 of char
// field : CPP`Expr
// ==> CPP`Stmt
TYPE_CPP_Stmt vdmcg::GenRecGetFieldOnThis(const TYPE_CPP_Expr & meth, const TYPE_CPP_Expr & field)
{
  return GenFctCallOnThis( meth, mk_sequence(field) );
}
...
// GenFctCallOnThis
// meth : CPP`Expr
// args seq os CPP`Expr
// == CPP`Expr
TYPE_CPP_Expr vdmcg::GenFctCallOnThis(const TYPE_CPP_Expr & meth, const SEQ<TYPE_CPP_Expr> & args)
{
  return vdm_BC_GenFctCall(GenObjectMemberAccessOnThis( meth ), args );
}

// GenObjectMemberAccessOnThis
// field : CPP`Expr
// ==> CPP`Expr
TYPE_CPP_Expr vdmcg::GenObjectMemberAccessOnThis(const TYPE_CPP_Expr & field)
{
#ifdef VDMPP
  if (vdm_CPP_isJAVA()) {
    return vdm_BC_GenObjectMemberAccess(GenThis(), field);
  }
  else
#endif // VDMPP
  {
    return vdm_BC_GenPointerToObjectMemberAccess(GenThis(), field);
  }
}
...
// elem : CPP`Expr
// ==> CPP`Stmt
TYPE_CPP_Stmt vdmcg::GenRecSetField(const TYPE_CPP_Expr & rec,
...
{
  return vdm_BC_GenExpressionStmt(GenFctCallOnThis( vdm_BC_GenIdentifier(ASTAUX::MkId(L"SetField")),
                                                    mk_sequence(field, elem) ));
}
...
{
  TYPE_CPP_Initializer ai;
#ifdef VDMPP
  if (vdm_CPP_isJAVA()) {
    ai = vdm_BC_GenAsgnInit(GenMapExpr(tmpMap));
  }
  else
#endif //VDMPP
    ai = vdm_BC_GenObjectInit(mk_sequence(tmpMap));
  ...
  TYPE_CPP_Stmt ifstmt(vdm_BC_GenIfStmt(vdm_BC_GenNot(GenSubSet(GenRng(tmpMap), GenDom(tmpMap))),
                     vdm_BC_GenBlock(mk_sequence(RunTime(L"The range is not a subset of the domain"))), nil));

  TYPE_CPP_Identifier mm2 (vdm_BC_GiveName (ASTAUX::MkId(L"mm")));
  TYPE_CPP_Identifier elem (vdm_BC_GiveName(ASTAUX::MkId(L"elem")));
  ...
  TYPE_CPP_Expr e_stop (vdm_BC_GenLt(count, n));
  SEQ<TYPE_CPP_Expr> inc;
  inc.ImpAppend(vdm_BC_GenPostPlusPlus(count));
  TYPE_CPP_Stmt forstmt (vdm_BC_GenForStmt (idcl, e_stop, inc, inner));

  TYPE_CPP_Stmt estmt (vdm_BC_GenBlock(type_dL().ImpAppend(ifstmt).ImpAppend(forstmt)));

  TYPE_CPP_Stmt ifstmt2 (vdm_BC_GenIfStmt(vdm_BC_GenEq(n, vdm_BC_GenIntegerLit(0)), estmt0, estmt));

  SEQ<TYPE_CPP_Stmt> s_l;
  s_l.ImpConc(GenMapDecl_DS (mm, ai));
  s_l.ImpAppend(ifstmt2);

  SEQ<TYPE_CPP_Stmt> rb_l;
  rb_l.ImpAppend(vdm_BC_GenIfStmt(vdm_BC_GenNeq(n, vdm_BC_GenIntegerLit(1)),
                                  vdm_BC_GenBlock(s_l), nil));
  return rb_l;
}
