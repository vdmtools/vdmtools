...
// 20130530 -->
//  Tuple res (theStackMachine().EvalPrint(rhs, Sequence(L"Create User Object")));
  AS2ASCII as2as;
  wostringstream wostr;
  //as2as.StateDesignator2ASCII(obj_name, wostr);
...
Set EvalState::CreateDestroySet( const Set & ref_s )
{
  ...
  for( bool dd = dom_ref_count.First(ref); dd; dd = dom_ref_count.Next(ref) )
  {
    ...
    if( total_ref_count.DomExists(ref) )
      ...
      total_count = Int(1);
    if( count == total_count ) ds.Insert(ref);
  }
  return ds;
}
...
//    desc_new.set_ref(desc.get_ref());
    desc_new.set_sem(sem_val);
//    desc_new.set_threadid(desc.get_threadid());
//    desc_new.set_dcip(desc.get_dcip());
...
void EvalState::GetObjectReferenceByID( const Int & id, Set& refs )
{
  ...
  for( bool bb = dom_obj_tab.First(ref); bb; bb = dom_obj_tab.Next(ref) )
  {
    ...
    {
      if( !refs.InSet( ref ) ) refs.Insert( ref );
      return;
    }
  }
}
...
{
  Set s;
//  Map d_objs( theStackMachine().d_objs );
  Set dom_d_objs (this->d_objs.Dom());
  Generic dom_g;
