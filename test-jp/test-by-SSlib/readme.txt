$Id: readme.txt,v 1.1 2005/11/25 07:05:38 vdmtools Exp $
test-by-SSLib -- 佐原伸作成のVDM++ライブラリーなどを使用したVDM++の回帰テスト用ディレクトリ
...
単体テストの実行方法
	事前条件
		...
		コードカバレージ情報を得たいときは、TeXを利用する。Sequence.texファイルを参考にして、清書したいファイルをincludeするファイルを作成する。
		Makefileを修正し、コードカバレージ情報を含んだ、清書されたPDFファイルを作成できるようにしておく。
	コマンドによる実行手順
		コマンドラインからvppdeを実行する。
		自動的に.vppdeファイルのスクリプトを実行し、構文チェック・型チェック・インタープリタによる実行・コードカバレージ情報を含んだ清書用TeXファイルの生成が行われる。
		清書されたPDFファイルが欲しいときは、makeする。
	GUIツール（vppgde）による実行手順
		vppgdeを実行する。
		test.prjファイルを読み込む。
		...
		コードカバレージ情報を含んだ清書ファイルを得たいときは、Managerウィンドウで Pretty Print（メニューかボタンから）すると .vpp.tex で終わるファイルが生成される。
		清書されたPDFファイルが欲しいときは、makeする。
